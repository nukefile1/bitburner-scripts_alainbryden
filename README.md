THIS IS A FORK, ORIGINAL HERE: https://github.com/alainbryden/bitburner-scripts
...
- `grep.js` - Use this to search one or all files for certain text. Handy if you are trying to figure out e.g. what script spend hashes, or care about the TIX api. Didn't use it once, not that important to me as of right now.
...
- `alias stop="home; kill autopilot.js ; kill daemon.js ; run kill-all-scripts.js"`
  - Quick way to start/stop the system. I personally now use `auto` instead of `start` for this alias (auto => autopilot.js). I use 'start', makes it easier to remember...
- `alias sscan="home; run scan.js"`
  - Makes it a little quicker to run this custom-scan routine, which shows the entire network, stats about servers, and provides handy links for jumping to servers or backdooring them. IMPORTANT SHORTCUT!
...
- `alias xp="run daemon.js -vx --tail --no-share"`
  - Runs daemon in a way that focuses on earning hack XP income as quickly as possible. Only practical when you have a lot of home-ram. aka Late-BN to get the world-daemon
